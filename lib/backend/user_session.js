...
function UserSession(userName, roomName, wsSession, sessionId, mediaPipeline) {
...
UserSession.prototype.createWebRtcEndpoint = function (callback) {
  ...
  if (this.mediaPipeline) {
    this.mediaPipeline.create('WebRtcEndpoint', function (error, webRtc) {
      ...
      logger.info('WebRtcEndpoint for user ' + self.userName + ', room ' +
        self.roomName + ' created.');
      callback();
    });
  }
};
...
UserSession.prototype.getRoomName = function () {
...
UserSession.prototype.receiveVideoFrom = function (sender, sdpOffer) {
  ...
  this.getEndpointForUser(sender, function (webRtc, error) {
    ...
    webRtc.processOffer(sdpOffer, function (error, sdpAnswer) {
      ...
      var params = {};
      params.id = 'receiveVideoAnswer';
      params.name = sender.getUserName();
      params.sdpAnswer = addMidsForFirefox(sdpOffer, sdpAnswer);
...
UserSession.prototype.sendMessage = function (message) {
  logger.info('USER ' + this.userName + ': Sending message ' + message);
  this.wsSession.send(JSON.stringify(message));
